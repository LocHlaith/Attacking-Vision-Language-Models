...
\section*{第1章 问题叙述}
\phantomsection
\addcontentsline{toc}{section}{第1章 问题叙述}
\markboth{第1章 问题叙述}{}

\subsection*{1.1 研究背景与现实意义}
\phantomsection
\addcontentsline{toc}{subsection}{1.1 研究背景与现实意义}
...
\subsection*{1.2 MobileNetV2 分类模型介绍}
\phantomsection
\addcontentsline{toc}{subsection}{1.2 MobileNetV2 分类模型介绍}
...
\end{bmatrix}.
\]
\noindent 其中 \(z_i(x)\) 称为第 \(i\) 类的 logit，也可理解为第 \(i\) 类的未归一化得分函数。MobileNetV2 的 TorchVision 源码实现中，分类器部分包含线性层，输出类别数由 \texttt{num\_classes} 控制；使用 ImageNet 权重时对应 ImageNet-1K 分类任务。([PyTorch Documentation][2])
...
\subsection*{1.3 MobileNetV2 的得分函数与损失函数}
\phantomsection
\addcontentsline{toc}{subsection}{1.3 MobileNetV2 的得分函数与损失函数}
...
\]
\noindent 其中，各层可能包括卷积、批归一化、ReLU6 激活、深度可分离卷积、倒残差结构、全局平均池化和线性分类层。MobileNetV2 的核心结构包括 inverted residual blocks 和 linear bottlenecks，这是其区别于传统卷积网络的重要设计。([PyTorch Documentation][3])
...
p_j(x)
=
\frac{\mathrm{e}^{z_j(x)}}{\sum_{k=1}^{K}\mathrm{e}^{z_k(x)}}.
\]
\noindent 其中
...
=
-\log
\frac{\mathrm{e}^{z_y(x)}}{\sum_{j=1}^{K}\mathrm{e}^{z_j(x)}}.
\]
\noindent 进一步化简：
...
=
-\left[
z_y(x)-\log\left(\sum_{j=1}^{K}\mathrm{e}^{z_j(x)}\right)
\right],
\]
\noindent 即
...
J(x,y)
=
\log\left(\sum_{j=1}^{K}\mathrm{e}^{z_j(x)}\right)
-
z_y(x).
\]
\noindent 当模型对真实类别 \(y\) 越自信时，\(p_y(x)\) 越大，\(J(x,y)\) 越小；当模型对真实类别 \(y\) 越不自信时，\(p_y(x)\) 越小，\(J(x,y)\) 越大。
...
-J(x+\delta,y).
\]
\noindent 当 \(\Phi\) 越小，等价于 \(J\) 越大，模型对真实类别的判断越不稳定。

\clearpage
\section*{第2章 解析法求解带二范数约束的非线性规划模型}
\phantomsection
\addcontentsline{toc}{section}{第2章 解析法求解带二范数约束的非线性规划模型}
\markboth{第2章 解析法求解带二范数约束的非线性规划模型}{}

\subsection*{2.1 算法设计：非线性规划建模与传统解析法}
\phantomsection
\addcontentsline{toc}{subsection}{2.1 算法设计：非线性规划建模与传统解析法}

本章将“攻击 MobileNetV2”建模为一个带二范数扰动约束的非线性规划问题，并分别讨论 Lagrange 函数法、Fritz John 条件和 Kuhn-Tucker 条件。

\subsubsection*{2.1.1 原始攻击模型}
\phantomsection
\addcontentsline{toc}{subsubsection}{2.1.1 原始攻击模型}
...
=
-\log
\frac{\mathrm{e}^{z_y(x+\delta)}}{\sum_{j=1}^{K}\mathrm{e}^{z_j(x+\delta)}}.
\]
\noindent 因此
...
\log
\left(
\sum_{j=1}^{K}\mathrm{e}^{z_j(x+\delta)}
\right).
\]
\noindent 于是，攻击模型可写为：
...
\subsubsection*{2.1.2 二范数平方约束的光滑性}
\phantomsection
\addcontentsline{toc}{subsubsection}{2.1.2 二范数平方约束的光滑性}
...
2I.
\]
\noindent 因此，二范数平方是处处可导、处处二阶可导的光滑凸函数。
...
\subsubsection*{2.1.3 Lagrange 函数法}
\phantomsection
\addcontentsline{toc}{subsubsection}{2.1.3 Lagrange 函数法}
...
\]
\noindent 其中 \(\lambda\) 为 Lagrange 乘子。
...
\frac{\nabla_xJ(x,y)}{|\nabla_xJ(x,y)|_2}.
\]
\noindent 该解不是原非凸问题的全局最优解，而是一阶近似模型下的全局最优解。对于原问题而言，它只能被视为局部近似解或满意解。

\subsubsection*{2.1.4 Fritz John 条件}
\phantomsection
\addcontentsline{toc}{subsubsection}{2.1.4 Fritz John 条件}
...
\subsubsection*{2.1.5 Kuhn-Tucker 条件}
\phantomsection
\addcontentsline{toc}{subsubsection}{2.1.5 Kuhn-Tucker 条件}
...
\subsubsection*{2.1.6 解析法所得解的性质分析}
\phantomsection
\addcontentsline{toc}{subsubsection}{2.1.6 解析法所得解的性质分析}
...
\nabla\Phi(\delta^*)+2\lambda\delta^*=0.
\]
\noindent 但由于 \(\Phi\) 是复杂非凸函数，该方程一般不能解析求全局解。
...
\]
\noindent 若使用一阶泰勒展开，则得到近似问题的全局最优解：
...
(2\lambda I-H)\delta=g.
\]
\noindent 该解是局部二次模型下的候选解，仍不能保证原问题全局最优。

\clearpage
\section*{第3章 数值法求解带二范数约束的非线性规划模型}
\phantomsection
\addcontentsline{toc}{section}{第3章 数值法求解带二范数约束的非线性规划模型}
\markboth{第3章 数值法求解带二范数约束的非线性规划模型}{}

\subsection*{3.1 算法设计：传统数值优化方法}
\phantomsection
\addcontentsline{toc}{subsection}{3.1 算法设计：传统数值优化方法}
...
\subsubsection*{3.1.1 罚函数与目标规划形式}
\phantomsection
\addcontentsline{toc}{subsubsection}{3.1.1 罚函数与目标规划形式}
...
D(\delta)=|\delta|_2^2.
\]
\noindent 若希望同时控制扰动大小并提高攻击成功程度，可构造目标规划模型：
...
\end{aligned}
\]
\noindent 其中 \(\eta\) 表示希望损失函数至少达到的水平。由于约束
...
\psi_\alpha(u)
=
\frac{1}{\alpha}\log(1+\mathrm{e}^{\alpha u}),
\]
\noindent 其中 \(\alpha>0\)。当 \(\alpha\) 较大时，
...
|\delta|_2^2
+
P\psi_\alpha(\eta-J(x+\delta,y)).
\]
\noindent 这使得传统数值优化方法更容易使用。

\subsubsection*{3.1.2 最速下降法}
\phantomsection
\addcontentsline{toc}{subsubsection}{3.1.2 最速下降法}
...
\right].
\]
\noindent 该方法每一步沿着使损失 \(J\) 增大的方向移动，然后将结果投影回允许扰动范围。

最速下降法计算简单，适合高维问题，但收敛速度可能较慢。由于目标函数非凸，若算法收敛，一般只能保证得到局部驻点或满意解，而不能保证全局最优。

\subsubsection*{3.1.3 牛顿法}
\phantomsection
\addcontentsline{toc}{subsubsection}{3.1.3 牛顿法}
...
\subsubsection*{3.1.4 拟牛顿法}
\phantomsection
\addcontentsline{toc}{subsubsection}{3.1.4 拟牛顿法}
...
\subsubsection*{3.1.5 共轭梯度法}
\phantomsection
\addcontentsline{toc}{subsubsection}{3.1.5 共轭梯度法}
...
\quad A\succ0.
\]
\noindent 其核心思想是构造一组关于 \(A\) 共轭的搜索方向：
...
\frac12s^\top H_ks.
\]
\noindent 若在该局部二次模型中使用共轭梯度法，则需要求解：
...
\]
\noindent 其中 \(\beta_k\) 可取 Fletcher-Reeves 形式：
...
\subsubsection*{3.1.6 数值法所得解的性质分析}
\phantomsection
\addcontentsline{toc}{subsubsection}{3.1.6 数值法所得解的性质分析}
...
\clearpage
\section*{第4章 基于对偶思想的攻击模型}
\phantomsection
\addcontentsline{toc}{section}{第4章 基于对偶思想的攻击模型}
\markboth{第4章 基于对偶思想的攻击模型}{}

\subsection*{4.1 算法设计：由损失约束构造对偶问题}
\phantomsection
\addcontentsline{toc}{subsection}{4.1 算法设计：由损失约束构造对偶问题}
...
\end{aligned}
\]
\noindent 该模型表示：在扰动预算不超过 \(\varepsilon\) 的情况下，尽可能增大分类损失。

本章采用另一种建模思路：把攻击成功程度作为约束，把扰动大小作为目标。该形式更接近传统运筹学中的“资源最小化问题”。

\subsubsection*{4.1.1 原始问题：最小扰动模型}
\phantomsection
\addcontentsline{toc}{subsubsection}{4.1.1 原始问题：最小扰动模型}
...
J(x+\delta,y)\ge \eta.
\]
\noindent 其中 \(\eta\) 是预先设定的攻击成功水平。若 \(\eta\) 足够大，则说明模型对真实类别 \(y\) 的分类信心明显下降。
...
\subsubsection*{4.1.2 Lagrange 函数}
\phantomsection
\addcontentsline{toc}{subsubsection}{4.1.2 Lagrange 函数}
...
\subsubsection*{4.1.3 KKT 条件}
\phantomsection
\addcontentsline{toc}{subsubsection}{4.1.3 KKT 条件}
...
\subsubsection*{4.1.4 对偶问题的数值求解}
\phantomsection
\addcontentsline{toc}{subsubsection}{4.1.4 对偶问题的数值求解}
...
\right].
\]
\noindent 外层：更新 \(\lambda\ge0\)，使约束满足。
...
\right].
\]
\noindent 外层可根据约束违反程度更新 \(\lambda\)。若
...
\right\}.
\]
\noindent 其中 \(\rho_t>0\) 是外层步长。

该方法属于传统 Lagrange 乘子或罚函数思想下的数值求解方法。

\subsubsection*{4.1.5 对偶方法的解的性质}
\phantomsection
\addcontentsline{toc}{subsubsection}{4.1.5 对偶方法的解的性质}
...
\clearpage
\section*{第5章 基于图论的结构化扰动模型：最小“张天羽”图案}
\phantomsection
\addcontentsline{toc}{section}{第5章 基于图论的结构化扰动模型：最小“张天羽”图案}
\markboth{第5章 基于图论的结构化扰动模型：最小“张天羽”图案}{}

\subsection*{5.1 算法设计：从像素扰动到图论优化}
\phantomsection
\addcontentsline{toc}{subsection}{5.1 算法设计：从像素扰动到图论优化}
...
\subsubsection*{5.1.1 基本变量定义}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.1 基本变量定义}
...
V=\{v_1,v_2,\ldots,v_m\}.
\]
\noindent 若两个像素在空间上相邻，例如上下左右相邻，则在它们之间连一条边：
...
\subsubsection*{5.1.2 “置黑扰动”的数学表达}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.2 “置黑扰动”的数学表达}
...
\]
\noindent 若像素位置 \(v_i\) 被置黑，则该位置三个通道均变为 0：

\[
x'_{i,\mathrm{R}}=0,\quad x'_{i,\mathrm{G}}=0,\quad x'_{i,\mathrm{B}}=0.
\]
\noindent 若不被扰动，则保持原值：

\[
x'_{i,c}=x_{i,c},
\quad c\in\{\mathrm{R},\mathrm{G},\mathrm{B}\}.
\]
\noindent 因此可写为：

\[
x'_{i,c}=(1-s_i)x_{i,c}.
\]
...
x'(s)=x\odot(1-s),
\]
\noindent 其中 \(s\) 在空间维度上对 RGB 三个通道复制，\(\odot\) 表示逐元素乘法。
...
\hat y(x'(s))\ne y.
\]
\noindent 等价地，可用 logit margin 表示：

\[
\max_{k\ne y}z_k(x'(s))-z_y(x'(s))\ge 0.
\]
\noindent 因此，最小黑色像素攻击问题可写为：
...
\subsubsection*{5.1.3 “张天羽”字形约束}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.3 “张天羽”字形约束}
...
T\subseteq V.
\]
\noindent 其中 \(T\) 表示在某个标准尺寸、标准位置、标准字体下，“张天羽”三个字对应的像素集合。
...
T(a).
\]
\noindent 若要求所有被置黑像素必须属于该模板，则有

\[
S\subseteq T(a).
\]
\noindent 若要求形成完整可识别文字，则不能任意删除笔画，需要引入连通性和笔画完整性约束。
...
\]
\noindent 其中每个 \(P_l\subseteq T(a)\) 是一个笔画像素集合。
...
\quad (i,j)\in E.
\]
\noindent 若节点 \(v_i\) 属于笔画 \(P_l\)，并且被选中，则需要有单位流到达。该类约束可写为网络流形式，但会显著增加模型规模。
...
\hat y(x'(T(a)))\ne y.
\]
\noindent 即寻找面积最小的“张天羽”模板，使攻击成功。

\subsubsection*{5.1.4 图论抽象}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.4 图论抽象}
...
\]
\noindent 若把每个节点 \(v_i\) 对攻击目标的贡献定义为权重 \(w_i\)，例如

\[
w_i
=
\left|
\frac{\partial J(x,y)}{\partial x_{i,\mathrm{R}}}
\right|
+
\left|
\frac{\partial J(x,y)}{\partial x_{i,\mathrm{G}}}
\right|
+
\left|
\frac{\partial J(x,y)}{\partial x_{i,\mathrm{B}}}
\right|,
\]
\noindent 则 \(w_i\) 表示该像素对损失函数的敏感程度。
...
\subsubsection*{5.1.5 数学难点一：非凸性}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.5 数学难点一：非凸性}
...
\subsubsection*{5.1.6 数学难点二：NP-hard}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.6 数学难点二：NP-hard}
...
s_i\in\{0,1\}.
\]
\noindent 该问题已经类似最小基数覆盖问题。若再加入笔画连通性、模板结构、多个区域覆盖要求，则可归约为集合覆盖、最小连通子图、Steiner tree 等经典 NP-hard 问题。
...
\hat y(x'(s))\ne y.
\]
\noindent 该约束并非简单线性约束，而是复杂黑箱或白箱函数约束。因此，原问题至少具有组合优化问题的复杂性。
...
\subsubsection*{5.1.7 数学难点三：可扩展性}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.7 数学难点三：可扩展性}
...
199808.
\]
\noindent 若对每条边设置流变量，模型规模会进一步扩大。

因此，直接精确求解完整 MIP 模型可能非常困难。必须使用降维、模板固定、候选点筛选、贪心策略或局部优化。

\subsubsection*{5.1.8 方法一：Saliency map + 贪心选择}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.8 方法一：Saliency map + 贪心选择}
...
w_i
=
\sum_{c\in\{\mathrm{R},\mathrm{G},\mathrm{B}\}}
\left|
\frac{\partial J(x,y)}{\partial x_{i,c}}
\right|.
\]
\noindent 若将像素置黑，则变化量为
...
|S_t|=t.
\]
\noindent 该方法计算简单，适合大规模图像。但由于每次只根据局部敏感度选择像素，可能忽略像素之间的组合效应。因此它一般得到满意解，而不能保证全局最优。

\subsubsection*{5.1.9 方法二：混合整数规划 MIP}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.9 方法二：混合整数规划 MIP}
...
\]
\noindent 但该约束高度非线性，不能直接放入普通 MIP。为此可以使用一阶线性化。
...
\sum_{i\in P_l,i\ne r_l}s_i.
\]
\noindent 同时，为了保证只有被选中的节点才能通过流量，可加入大 \(M\) 约束：
...
u_{ij}^{(l)}\le M s_j.
\]
\noindent 这样可保证选择的笔画像素具有连通性。

MIP 方法比贪心法更接近全局组合优化，但由于攻击约束经过线性化，求得的是线性近似模型下的最优解。对于原始神经网络模型，它仍然是满意解或局部近似解。

\subsubsection*{5.1.10 方法三：图论启发式求解}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.10 方法三：图论启发式求解}
...
\arg\min_{i\in S}r_i.
\]
\noindent 若删除 \(i^*\) 后仍满足字形结构并攻击成功，则接受删除：
...
\subsubsection*{5.1.11 图论模型的解的性质分析}
\phantomsection
\addcontentsline{toc}{subsubsection}{5.1.11 图论模型的解的性质分析}
...
\end{aligned}
\]
\noindent 则这是一个非凸、离散、组合优化问题，通常无法有效求得全局最优。
